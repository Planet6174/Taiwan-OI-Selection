...
好景不长，他便发现因为权重 (weight) 设定错误的关系，上面答案并不是正确的最小生成树。阿紫决定要将原边权做一些调整，好让设定的正确答案是真正的最小生成树。另外，为了便于阅卷，他希望调整完权重之后，图中只有唯一一棵最小生成树。为了避免麻烦，阿紫希望调整权重的边尽可能少，而且他不希望调整最小生成树上的边（上图中以粗线标示的边）。请问有多少条边需要调整权重？
